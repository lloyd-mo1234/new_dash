...
def debug_print_trade_pnl_arrays():
    """
    DEBUG FUNCTION: Print PnL arrays for all trades in terminal
    Format: dates x columns (one per position) + total
    This entire function can be deleted after debugging
    """
    try:
        print("\n" + "=" * 100)
        print("🐛 DEBUG: TRADE PNL ARRAYS OUTPUT")
        print("=" * 100)
        
        # Check if curves are loaded
        if not is_curves_loaded():
            print("⚠️ Curves not loaded - cannot calculate PnL arrays")
            print("=" * 100 + "\n")
            return
        ...
        print(f"✅ Initialized {total_positions} positions across {successful_trades} trades\n")
        
        # Get futures data once for all trades
        futures_instruments = get_futures_instrument_names(portfolio)
        futures_tick_data = None
        historical_prices = None
        
        if futures_instruments:
            print(f"📈 Getting futures data for {len(futures_instruments)} instruments...")
            ...
            historical_prices = get_futures_history(portfolio, start_date, end_date)
            print(f"✅ Futures data loaded from {start_date} to {end_date}\n")
        ...
        for trade_id, trade in portfolio.trades.items():
            ...
            print(f"   Primary positions: {len(trade.positions)}")
            print(f"   Secondary positions: {len(trade.positions_secondary)}")
            print("-" * 100)
            
            # Calculate array PnL
            result = trade.calculate_array_pnl(
            ...
            if not pnl_array:
                print("⚠️ No PnL array data")
                continue
            
            # Get individual position PnL arrays
            num_positions = len(trade.positions)
            position_pnl_arrays = []
            
            for idx, position in enumerate(trade.positions):
                if hasattr(position, 'calculate_array_pnl'):
                    if hasattr(position, 'futures_built'):
                        pos_result = position.calculate_array_pnl(futures_tick_data, historical_prices)
                    else:
                        pos_result = position.calculate_array_pnl()
                    
                    if not pos_result.get('error'):
                        pnl_dict = {date: pnl for date, pnl in pos_result['pnl_array']}
                        position_pnl_arrays.append({
            ...
            # Build DataFrame
            df_data = []
            for date, total_pnl in pnl_array:
                row = {'Date': date}
                
                for idx, pos_info in enumerate(position_pnl_arrays):
                    col_name = f"Pos_{idx}"
                    row[col_name] = pos_info['pnl_dict'].get(date, 0.0)
            ...
            # Print first 10 rows
            print(f"\n📊 First 10 dates (Positions: {num_positions}):")
            print(df_display.head(10).to_string(index=False, max_colwidth=15))
            
            if len(df) > 20:
                print(f"\n   ... ({len(df) - 20} more rows) ...")
                print(f"\n📊 Last 10 dates:")
                print(df_display.tail(10).to_string(index=False, max_colwidth=15))
            elif len(df) > 10:
                print(f"\n📊 Remaining dates:")
                print(df_display.iloc[10:].to_string(index=False, max_colwidth=15))
            
            # Print summary stats
            print(f"\n📈 Summary Statistics:")
            print(f"   Total dates: {len(df)}")
            print(f"   Total PnL - Min: ${df['Total'].min():,.2f}, Max: ${df['Total'].max():,.2f}, Mean: ${df['Total'].mean():,.2f}")
            
            for idx in range(num_positions):
                col_name = f"Pos_{idx}"
                if col_name in df.columns:
                    print(f"   {col_name} - Min: ${df[col_name].min():,.2f}, Max: ${df[col_name].max():,.2f}, Mean: ${df[col_name].mean():,.2f}")
        
        print("\n" + "=" * 100)
        print("🐛 DEBUG: TRADE PNL ARRAYS OUTPUT COMPLETE")
        print("=" * 100 + "\n")
        
    except Exception as e:
        print(f"\n❌ DEBUG FUNCTION ERROR: {e}")
        import traceback
...
def get_trades():
    ...
    try:
        # 🐛 DEBUG: Call debug function when trades are loaded
        debug_print_trade_pnl_arrays()
